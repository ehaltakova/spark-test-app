/**
 * Slide Albums Manager.
 * Handles CRUD of slide albums.
 * Calls the Slide Albums REST API.
 * Keep an instance of the UI object representing the slide albums list and calls its methods as REST WS callbacks.
 * 
 ...
 */
	...
	SlideAlbumsManager = function () {
        ...
        SlideAlbumsManager.prototype.getSlideAlbums = function () { 
    		var url = "http://localhost:4567/spark/api/public/slidealbums"; //API_BASE_URL + '/api/getSlideAlbums';
    		var type = 'POST';	 
    		var data = {}; // request data
    		var callback = (function(response){ return this.slideAlbumsList.displaySlideAlbums(response);}).bind(this);	
        ...
        SlideAlbumsManager.prototype.createSlideAlbum = function(slideAlbum) {  
    		...
    		var callback = (function(response){ return this.slideAlbumsList.addSlideAlbum(response); }).bind(this);    		
    		this.restApiUtil.execute(url, type, data, callback, true);	   	
        };
        ...
        SlideAlbumsManager.prototype.editSlideAlbum = function (slideAlbum) {       
    		...
    		var type = 'POST';	 
    		var data = slideAlbum; // request data
    		var callback = (function(response){ return this.slideAlbumsList.updateSlideAlbum(response); }).bind(this);
    		this.restApiUtil.execute(url, type, data, callback, true);
        };
        ...
        SlideAlbumsManager.prototype.deleteSlideAlbum = function (title, customer) {    
        	var url = "http://localhost:4567/spark/api/public/slidealbums/delete"; //API_BASE_URL + '/api/deleteSlideAlbum';
    		var type = 'POST';	 
    		var data = {'title' : title, 'customer': customer}; // request data
    		var callback = (function(response){ return this.slideAlbumsList.removeSlideAlbum(title, customer); }).bind(this);  		
    		this.restApiUtil.execute(url, type, data, callback, true);	    		
        };
		
/**
 * Slide Albums UI List.
 * Represents a table with slide albums.
 * Provides UI for slide album CRUD operations.
 */	
	SlideAlbumsList = function (slideAlbumsMgr) {
    	...
    	/**
        *  Get Slide Album WS callback - display the slide albums being retrieved by the REST API.
        */
        SlideAlbumsList.prototype.displaySlideAlbums = function (resp) {
       
        	this.slideAlbumsFilter.initialize(); // filter should be initialized before the table bacause of custom filter state saving for the table!

//          HOW THE CUSTOM FILTER IS APPLIED AND PRESERVED
//        	1. Filter drop down is intialized from the 
//        	2. table is initialized, state is retrieved: if there are customers saved in the state the filter is pre-selected with them;
...
//        	3. table is redrawn so the filter to be applied
        	...
        	var slideAlbums = resp['slideAlbums'];
        	...
        	var table = $('#slideAlbumsTable').DataTable( {
        		...
        		"aoColumnDefs": [
    						   { "width": "7%", "targets": 4, "sortable": false, "searchable": false },
    						   { "width": "7%", "targets": 5, "sortable": false, "searchable": false },
        		               { "width": "7%", "targets": 6, "sortable": false, "searchable": false },
        		               { "width": "20%", "targets": 1 },
        		...
        		"stateLoadParams": function (oSettings, oData) { // load selected customers from the jquery datatables state and preselect the filter drop-down
        			$("#selectCustomer").val(oData['customers'] != null && oData['customers'].length > 0 ? oData['customers'] : sessionMgr.getUserContext()['customers']);
        	...
        	if(sessionMgr.getUserContext()['customers'].length > 1) {
       		...
       		$('#slideAlbumsCount').html(localize("You have {slideAlbumsCount} slide albums in your workspace", { slideAlbumsCount: slideAlbumsObjects.length}));    			
        ...
        */
        SlideAlbumsList.prototype.addSlideAlbum = function (response) {
        	...
        	var slideAlbum = response['slideAlbum'];
        	var table = $('#slideAlbumsTable').DataTable();	
        	...
        	var countNr = table.data().length;
           	$('#slideAlbumsCount').html(localize("You have {slideAlbumsCount} slide albums in your workspace", { slideAlbumsCount: countNr}));    			
        ...
        /**
    	 *  Delete Slide Album WS callback - removes the corresponding slide album row from the table.
    	 */
        SlideAlbumsList.prototype.removeSlideAlbum = function (title, customer) {
            ...
            $('#slideAlbumsCount').html(localize("You have {slideAlbumsCount} slide albums in your workspace", { slideAlbumsCount: countNumber}));    			
        };
 ...
 */
	...
	CreateSlideAlbumDialog = function (slideAlbumsMgr) {
    	...
    	CreateSlideAlbumDialog.prototype.initialize = function() {   			    		
    		...
    		// set user context needed to the request as hidden form filed
    		$("#sessionToken").val(sessionMgr.getUserContext()['sessionToken']);
    		// bind click event
    	...
    	CreateSlideAlbumDialog.prototype.attachFileUploader = function() {
    		...
    		$(function () {
    		    $('#fileupload').fileupload({
    		    	...
    		    	'url': 'http://localhost:4567/spark/api/slidealbums/create', //config['api_base_url'] + '/src/upload/',
    		        'add': function (e, data) {		
    		        ...
    		        'done': function (e, data) {
    		        	createSADialog.slideAlbumsMgr.slideAlbumsList.addSlideAlbum(data.result);
    		        }, 
    	...
    	CreateSlideAlbumDialog.prototype.validateForm = function() {
    		...
    		var filename = $("#selectedFileName").val(); // this will be set if file was uploaded with the file uploader
    	...
    	CreateSlideAlbumDialog.prototype.submitForm = function(e, data) {	
    		...
    		var validationMessages = createSADialog.validateForm();
     		if(validationMessages.length > 0) {
     			displayNotification('danger', validationMessages, '#formErrors', false); // display errors if validation fials
     		} else {
	        	data.submit();
    	...
    	/**
    	 * Fill in customers single drop-down with the customers names assigned to current user
    	 */
    	CreateSlideAlbumDialog.prototype.populateCustomersDropDown = function() {
    		var customers = sessionMgr.getUserContext()['customers'];
    		if(customers == null) {
    			return;
    		}
    		var customersOptionsStr = "";
    		for(var i=0; i<customers.length; i++) {
    			customersOptionsStr += "<option value='" + customers[i] + "'>" + customers[i] + "</option>";
    		}
    		$("#customerSelect").html(customersOptionsStr);
    	};
    		...
    		$(function () {
    		    $('#editFileupload').fileupload({
    		        ...
    		        'done': function (e, data) {
    		        	...
    		        	$("#editSAForm").submit(); // if upload is successfull submit the create form
    		        },
    		        'error': function (e, data) {
...
//    	    		errorMessages['messages'].push(localize('Title contains invalid characters. Allowed are letters, digits and the following symbols: "(", "), "_", "-".'));
//    	    	}
    	    }
...
//   	        		errorMessages['messages'].push(localize('File name contains invalid characters. Allowed are letters, digits and the following symbols: "(", "), "_", "-".'));
//   	        	}
    	    }
    	...
    	/**
    	 * Pre-select the dialog with the data of the slide album to be edited and show it
    	 */
...
//    		console.log("column index: " + sortingColumnIndex + " in " + order + " order");
...
/**
 * Slide Albums Customers Filter.
 * UI component providing the ability to filter the table by one or by mutliple customers.
 */
	...
	SlideAlbumsFilter = function () {
    	...
    	SlideAlbumsFilter.prototype.initialize = function() { 	
    		...
    		var customersList = sessionMgr.getUserContext()['customers'];
    		if(customersList == null) {
    			return;
    		}
    		
    		var slideAlbumsFilter = this;
    		var customerSelectHtml = '<div style="float:right; margin-right:15px;">';
			customerSelectHtml += '<select id="selectCustomer" multiple="multiple">';
		    for(var i=0; i<customersList.length; i++) {
		    	customerSelectHtml += '<option value="' + customersList[i] + '" selected>' + customersList[i] + "</option>";
		    }
			customerSelectHtml += '</select>' + '</div>';
			if(customersList.length > 1) {
				$("#customerDropDown").html(customerSelectHtml);
			}
    	...
    	/**
    	 * Show/Hide slide albums for the specified customer 
    	 * Note: not used
    	 */
    	SlideAlbumsFilter.prototype.filterList = function(customerName, checked) {
			...
			if(checked) {
				...
				if(slideAlbumsFilter.rows[customerName] != null && slideAlbumsFilter.rows[customerName].length > 0) {
					...
					table.rows().every( function ( rowIdx, tableLoop, rowLoop ) {
					    var d = this.data();
					 	if(d.customer != customerName && $.inArray(d.customer, selectedCustomers) == -1) {
					 		rowsToDelete.push(this.node());
					 		if(slideAlbumsFilter.rows[d.customer] == null) {
					 			slideAlbumsFilter.rows[d.customer] = [];
					 		}
...
function buildFilesColumn(files, filePath) {
	...
	// add the svg first
	filesCol += '<a href="' + filePath + svg['name'] + "."+ svg['ext'] + '"> <img title="' + svg['name'] + "."+ svg['ext'] + '" class="file-type-icon" data-filename="'+ svg['name'] + '" src="images/' + svg['ext'] + '.png" /> </a>';
	// add the animationfiles
	for(var j=0; j<animationFiles.length; j++) {
		var pathToFile = filePath + animationFiles[j]['name']+"."+ animationFiles[j]['ext'];
		filesCol += '<a href="' + pathToFile + '"> <img title ="' + animationFiles[j]['name']+"."+ animationFiles[j]['ext'] + '" class="file-type-icon" src="images/' + animationFiles[j]['ext'] + '.png" /></a>';
	}
	// add the json file at the end if exists
	if(json != null) {
		filesCol += '<a href="' + filePath + json['name'] + "."+ json['ext'] + '"> <img title="' + json['name'] + "."+ json['ext'] + '" class="file-type-icon" src="images/' + json['ext'] + '.png" /> </a>';
	}
	filesCol += '</div>';
	return filesCol;
}
...
function buildRowData(slideAlbum) {
	...
	var viewURL = APP_BASE_URL + '/slidealbum?' + encodeURIComponent('title=' + slideAlbum['title'] + '&customer=' + slideAlbum['customer'] + '&readOnly=true');
	var openURL = APP_BASE_URL + '/slidealbum?' + encodeURIComponent('title=' + slideAlbum['title'] + '&customer=' + slideAlbum['customer'] + '&readOnly=false');
	var viewIcon = '<a href="' + viewURL + '" title="' + localize("View slide album in read-only mode") + '" class="view-slide-album" data-name="'+ slideAlbum['title'] + '" data-customer="'+ slideAlbum['customer'] + '"> <span class="glyphicon glyphicon-eye-open"></span> </a>';
	var titleCol = '<div class="slide-album-title">' + lockedIcon + slideAlbum['title'] + '  ' + viewIcon + '</div>';
	var contentCol = '<a href="' + openURL +'" title="' + localize("Open to edit content") + '" class="open-slide-album btn btn-warning btn-sm" data-name="'+ slideAlbum['title'] + '" data-customer="'+ slideAlbum['customer'] + '"> <span class="glyphicon glyphicon-cog"></span> </a>';
	var editCol = '<a href="#" title="' + localize("Edit slide album") + '" class="edit-slide-album btn btn-danger btn-sm"" data-name="'+ slideAlbum['title'] + '" data-customer="'+ slideAlbum['customer'] + '"> <span class="glyphicon glyphicon-edit"></span> </a>';
	var deleteCol = '<a href="#"  title="' + localize("Delete slide album") + '" class="delete-slide-album btn btn-primary btn-sm" data-name="'+ slideAlbum['title'] + '" data-customer="'+ slideAlbum['customer'] + '"> <span class="glyphicon glyphicon-trash"></span> </a>';
	var date = formatDate(new Date(slideAlbum['modificationDate']*1000));
	var customerCol = slideAlbum['customer'];
	var filePath = config['workspace_folder_path'] + "/" + slideAlbum['customer'] +"/" + slideAlbum['title'] + "/";
	var filesCol = buildFilesColumn(slideAlbum['files'], filePath);
	var rowData = {'title': titleCol, 'files': filesCol, 'customer': customerCol, 'modificationDate': date, 'content': contentCol, 'edit': editCol, 'delete': deleteCol, 'modificationDateTime': slideAlbum['modificationDate'], 'titleStr': slideAlbum['title']};
	return rowData;
}

function formatDate(date) {
//	var formattedDate = date.toLocaleDateString(config['locale'], {day: 'numeric', month : "short", year: 'numeric'});
	var monthNames = [
